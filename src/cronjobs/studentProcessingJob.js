      ...
      const studentPromises = allStudents.map(async (student) => {
        ...
        if (student.accountStatus.status == "regular") {
          if (
            ...
            await annualActiveStudentsModel.create(newStudent);
          } else if (
            student.paymentStatus.addmissionDueStatus == true &&
            student.paymentStatus.consequentDueStatus == false
          ) {
            await annualDueStudentsModel.create(newStudent);
          } else if (
            student.paymentStatus.addmissionDueStatus == true &&
            student.paymentStatus.consequentDueStatus == true
          ) {
            await annualPendingStudentsModel.create(newStudent);
          }
        } else {
          await annualIrregularStudentsModel.create(newStudent);
        }
      });
